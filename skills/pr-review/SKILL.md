...
2. **Launch the `pr-reviewer` sub-agent asynchronously** — capture the run ID for later resumption:
...
3. **Do not wait or block.** The agent will report back asynchronously when its review is ready. Continue responding to the user in the meantime. Note the run ID — if the agent completes before a decision is made, you can resume it.

4. **If the agent contacts you for approval** via `contact_supervisor`, reply quickly with one of:
...
5. **If the agent's `contact_supervisor` times out** (~1–2 min), the agent exits gracefully but leaves the report file on disk. You have two options:
...
6. **When the user says "post the review of PR #<number>" and you have the run ID:**
...
7. **When the user says "clean up review <number>":**
...
8. **Follow-up questions** — if the agent can be resumed, the user can ask about the codebase or request report changes. If it can't, the report file is still there for reference.
